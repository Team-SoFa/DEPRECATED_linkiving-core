...
public class ArticleMangeService {
	...
	public ListRes<ArticleRes> getRecommend(Member member) {
		...
		for (Article article : articleListMap.keySet()) {
			double similarity = calculateAdjustedSimilarity(userTagIdStatistics, article.getViews(),
				articleListMap.get(article), totalUserTagCount);
	...
	private double calculateAdjustedSimilarity(Map<Long, Long> userTagStatistics, Long views, List<Long> tagIdList,
		Long totalUserTagCount) {
	...
	private double calculateSimilarity(Map<Long, Long> userTagIdStatistics, List<Long> tagIdList,
		Long totalUserTagCount) {
